...
public sealed class MER0013RespectBackendLayerBoundariesAnalyzer : DiagnosticAnalyzer
{
    ...
    private static readonly LocalizableString MessageFormat =
        "Remove this dependency because it crosses a documented application layering rule";
    ...
    {
        if (context.Node is not CompilationUnitSyntax compilationUnit) return;
    ...
    {
        if (MeridianAnalyzerSyntaxHelpers.PathContains(filePath, "/Core/")) return BackendProject.Core;

        if (MeridianAnalyzerSyntaxHelpers.PathContains(filePath, "/Infrastructure/"))
            return BackendProject.Infrastructure;

        if (MeridianAnalyzerSyntaxHelpers.PathContains(filePath, "/Shared/")) return BackendProject.Shared;

        if (MeridianAnalyzerSyntaxHelpers.PathContains(filePath, "/Analytics/")) return BackendProject.Analytics;
