...
public sealed class MER0024AvoidStringExtensionsInQueryableAnalyzer : DiagnosticAnalyzer
{
    ...
    private static readonly LocalizableString Title =
        "Avoid shared string extension guards inside IQueryable predicates";

    private static readonly LocalizableString MessageFormat =
        "Replace {0} with a query-translatable guard for IQueryable filters";
    ...
    private static void AnalyzeInvocation(SyntaxNodeAnalysisContext context)
    {
        ...
        if (MeridianAnalyzerRuleHelpers.IsTestPath(invocation.SyntaxTree.FilePath)) return;

        if (!IsTargetStringExtensionInvocation(invocation)) return;

        if (!IsQueryableContext(invocation)) return;
    ...
    private static bool IsTargetStringExtensionInvocation(InvocationExpressionSyntax invocation)
    {
        ...
        if (memberAccess.Name.Identifier.ValueText is not ("IsNullOrEmpty" or "IsNullOrWhiteSpace")) return false;

        if (memberAccess.Expression is IdentifierNameSyntax
            {
                Identifier.ValueText: "string" or "String"
            }) return false;
