...
public sealed class MER0017AvoidUnboundedEfMaterializationAnalyzer : DiagnosticAnalyzer
{
    ...
    private static readonly LocalizableString MessageFormat =
        "Review this async materialization for an explicit Where/Take/Skip bound";
    ...
    private static bool ChainContainsInvocation(SyntaxNode node, string[] methodNames)
    {
        return node.DescendantNodesAndSelf()
            ...
            .Any(invocation => methodNames.Any(name =>
                string.Equals(MeridianAnalyzerRuleHelpers.GetSimpleInvocationName(invocation), name,
                    StringComparison.Ordinal)));
    }

    private static bool ExpressionHasVisibleBound(AnalysisScope scope, ExpressionSyntax receiver,
        ...
        return node.DescendantNodesAndSelf()
            ...
            .Any(invocation =>
                invocation.ArgumentList.Arguments.Any(argument =>
                    ContainsRawSqlBoundingTerm(argument.Expression.ToString())));
    }
    ...
    private static bool ReceiverComesFromBoundedLocal(AnalysisScope scope, ExpressionSyntax receiver)
    {
        if (receiver is not IdentifierNameSyntax identifierName ||
            scope.SemanticModel.GetSymbolInfo(identifierName, scope.CancellationToken).Symbol is not ILocalSymbol
                localSymbol)
            return false;

        if (!scope.TryEnterSymbol(localSymbol, out var localScope)) return false;
    ...
    {
        if (receiver is not InvocationExpressionSyntax invocation) return false;
    ...
    private static bool ReceiverComesFromSyntacticallyBoundedMethod(AnalysisScope scope,
        InvocationExpressionSyntax invocation)
    {
        ...
        if (containingClass is null || string.IsNullOrEmpty(invocationName)) return false;
    ...
    private static bool ReceiverChainComesFromVisibleBound(AnalysisScope scope, ExpressionSyntax receiver)
    {
        return receiver switch
        {
            ...
            MemberAccessExpressionSyntax memberAccess => ReceiverChainComesFromVisibleBound(scope,
                memberAccess.Expression),
            _ => false
        };
    }

    private static bool InvocationReceiverComesFromVisibleBound(AnalysisScope scope,
        InvocationExpressionSyntax invocation)
    {
    ...
    private static bool MethodReturnsBoundedQuery(AnalysisScope scope, MethodDeclarationSyntax method)
    {
        ...
        if (methodScope.SemanticModel.GetDeclaredSymbol(method, methodScope.CancellationToken) is IMethodSymbol
                methodSymbol &&
        ...
        return method.Body?.Statements
            ...
            .Any(statement =>
                statement.Expression is { } expression && ExpressionHasVisibleBound(methodScope, expression)) == true;
    }
    ...
    {
        internal AnalysisScope(Compilation compilation, SemanticModel semanticModel,
        ...
        internal AnalysisScope ForSyntaxTree(SyntaxTree syntaxTree)
        {
            return SemanticModel.SyntaxTree == syntaxTree
                ? this
                : new AnalysisScope(Compilation, Compilation.GetSemanticModel(syntaxTree), CancellationToken,
                    ActiveSymbols);
        }
