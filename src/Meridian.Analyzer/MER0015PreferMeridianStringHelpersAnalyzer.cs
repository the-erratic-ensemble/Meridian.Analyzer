...
public sealed class MER0015PreferMeridianStringHelpersAnalyzer : DiagnosticAnalyzer
{
    ...
    private static readonly LocalizableString MessageFormat =
        "Prefer Shared string helpers for in-memory string normalization";
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (sourceType is not null) return IsIQueryableType(sourceType);

        var queryType = semanticModel.GetTypeInfo(queryExpression, cancellationToken).Type;
        if (queryType is not null) return IsIQueryableType(queryType);
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (invocation is null || !IsKnownQueryMethodName(invocation)) return false;
        ...
        CancellationToken cancellationToken)
    {
        ...
        var typeSyntax = symbol switch
        {
            ...
            ILocalSymbol local => local.DeclaringSyntaxReferences
                ...
                .Select(declarator => declarator.Parent?.Parent is VariableDeclarationSyntax declaration
    ...
    {
        if (receiver is not IdentifierNameSyntax identifierName) return false;
        ...
        return containingMethod.ParameterList.Parameters.Any(parameter =>
            string.Equals(parameter.Identifier.ValueText, identifierName.Identifier.ValueText,
